...
# Sorgente temporale scalata
def P_bar(tau):
    return torch.ones_like(tau)  # placeholder costante; personalizza se necessario

# Sorgente di bordo scalata
def v_bar(tau):
    return torch.ones_like(tau)  # placeholder per v̄(τ)

# Coefficienti IC polinomiale, da personalizzare
b1, b2, b3, b4 = 0.1, 0.1, 0.1, 0.1

# Definizione del residuo PDE scalato (Bio-Heat)
def pde(x, y):
...
#####################
# Condizione di Robin al bordo sinistro (X=0)
def robin_bc_left(x, on_boundary):
    return on_boundary and np.isclose(x[0], 0.0)

bc_left = dde.icbc.OperatorBC(
    geomtime,
    lambda x, y, _: dde.grad.jacobian(y, x, i=0, j=0) - a5 * (v_bar(x[:, 1:]) - y),
    robin_bc_left,
)

# Dirichlet al bordo destro (X=1)
# bc_right = dde.icbc.DirichletBC(
#     geomtime, lambda x: 0.0, lambda x, on_boundary: on_boundary and np.isclose(x[0], 1.0)
# )

# # Condizione iniziale polinomiale
# ic = dde.icbc.IC(
#     geomtime,
#     lambda x: (
#         b1 * x[:, 0:1] ** 3
...
def initial_cond(x):
    out=b1 * x[:, 0:1] ** 3 + b2 * x[:, 0:1] ** 2+ b3 * x[:, 0:1]+ b4
    return out
...
def  output_transform(x, y): #prende input e output della rete
    return x[:,1] * (1 - x[:,0]) * y + initial_cond(x)

net.apply_output_transform(output_transform)

# TRAINING PINN: stampo iterazioni di loss su train e test
model.compile("adam", lr=1e-3)
losshistory, train_state = model.train(iterations=20000, display_every=200)
model.compile("L-BFGS") #per sistemare e smussare gli errori residui

losshistory, train_state = model.train(model_save_path="./tesi/models/pinn", display_every=200)
...
data = np.load("misure_dataset_1.npz")
t = data["t"].flatten()
x = data["x"].flatten()
xx, tt = np.meshgrid(x, t)
